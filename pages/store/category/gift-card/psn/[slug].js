...
//         // `api/gift-cards?filters[slug][$eq]=${slug}&populate[seo][populate]=*`
//         `api/gift-cards?filters[slug][$eq]=${slug}&populate=*`
//     );
...
export async function getServerSideProps({ params }) {
    ...
    try {
        ...
        if (cachedData) {
            console.log("✅ Gift Card Cache HIT");
    ...
    console.log("❌ Gift Card Cache MISS");

    // 2. FETCH FROM STRAPI
    const giftCardRes = await fetchFromStrapi(
        `api/gift-cards?filters[slug][$eq]=${slug}&populate=*`
    );
    ...
    const props = {
        product: giftCardRes?.data?.[0] || null,
        regionsData: regionsRes?.data || [],
    };
        ...
        <>
            ...
            <div className="min-h-screen p-4 lg:p-6">
                ...
                <div className="max-w-[1500px] mx-auto grid grid-cols-1 md:grid-cols-1 lg:grid-cols-[240px_1fr_320px] xl:grid-cols-[260px_1fr_360px] 2xl:grid-cols-[260px_1fr_380px] gap-4 lg:gap-6 xl:gap-8">
                    ...
                    {/* Left: Cover Image - Fixed width for laptop and up */} {/* Left: Cover Image */}

                    <div className="w-full flex justify-center lg:justify-start">

                        <div className="relative w-full max-w-[240px] lg:max-w-[220px] xl:max-w-[240px] 2xl:max-w-[260px] flex-shrink-0">
                ...
                {product.releaseDate && (<div className="bg-[#1a1a1a] text-white p-4 lg:p-6 rounded-xl mt-4 lg:mt-8 border border-[#2a2a2a] text-xs lg:text-sm space-y-3 lg:space-y-4 max-w-[1500px] mx-auto">
                    ...
                    <div className="flex  justify-around md:grid-cols-4 gap-4 lg:gap-6">
                        ...
                        {product.releaseDate && (<div>
                            ...
                            <p className="text-white">{formatDate(product.releaseDate)}</p>
                        </div>)}
                        {product.ageimage && (<div>
