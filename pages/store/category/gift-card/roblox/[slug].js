...
export default function ProductPage({ product, regionsData }) {
    ...
    const Type = "roblox";     // As slug page parent folder are hardcoded psn so we can set type psn here. We directly inject it here...
            ...
            <div className="max-w-[1500px] mx-auto grid grid-cols-1 md:grid-cols-1 lg:grid-cols-[260px_1fr_380px] gap-4 lg:gap-6 xl:gap-8">
                ...
                <div className="flex flex-col gap-2 mx-auto xl:mx-0">   {/* mx-auto is the problem we need to fix it leater */}
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-sm mt-2.5">
                        <span className="bg-[#E2231A] px-2.5 py-1 rounded font-medium text-white">{product.item_type}</span>
                ...
                >
                    ...
                    <div className="flex gap-3.5">
                        {Tags.map((tag) => (
                            <span
                                key={tag.id}
                                className="rounded-2xl bg-[#2a2a2a] hover:bg-[#333] transition-all h-[35px] w-[120px] flex items-center justify-center"
                            >
                                {tag.Tags}
                            </span>
                        ))}
